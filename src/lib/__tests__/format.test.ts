...
describe("formatMoney", () => {
  it("uses el-GR convention with a non-breaking space before €", () => {
    expect(formatMoney(100)).toBe(`100,00${NBSP}€`);
    expect(formatMoney(0)).toBe(`0,00${NBSP}€`);
  });

  it("groups thousands with a dot", () => {
    expect(formatMoney(1234.56)).toBe(`1.234,56${NBSP}€`);
    expect(formatMoney(1234567.89)).toBe(`1.234.567,89${NBSP}€`);
    expect(formatMoney(999)).toBe(`999,00${NBSP}€`);
  });

  it("handles negatives", () => {
    expect(formatMoney(-45.5)).toBe(`-45,50${NBSP}€`);
  });

  it("rounds half up at the cent", () => {
    expect(formatMoney(1.005)).toBe(`1,01${NBSP}€`);
    expect(formatMoney(2.675)).toBe(`2,68${NBSP}€`);
  });
});
...
describe("formatPrice", () => {
  it("is always VAT-inclusive — there is no net/gross toggle", () => {
    expect(formatPrice(100)).toBe(`124,00${NBSP}€`);
    expect(formatPrice(100, { vatRate: 13 })).toBe(`113,00${NBSP}€`);
    expect(formatPrice(100, { partnerFactor: 0.88 })).toBe(`109,12${NBSP}€`);
  });

  it("matches a real product from the live API (net 63.92, VAT 24%)", () => {
    expect(formatPrice(63.92, { vatRate: 24 })).toBe(`79,26${NBSP}€`);
  });
});

describe("formatNet", () => {
  it("stays net — for ERP payloads and /admin only", () => {
    expect(formatNet(100)).toBe(`100,00${NBSP}€`);
    expect(formatNet(100, { partnerFactor: 0.88 })).toBe(`88,00${NBSP}€`);
  });
});

describe("savingsOf", () => {
  it("returns null when there is no real discount", () => {
    expect(savingsOf(100, 100)).toBeNull();
    expect(savingsOf(80, 100)).toBeNull();
  });

  it("computes the saving gross, like every other displayed price", () => {
    expect(savingsOf(100, 88)).toEqual({
      amount: 14.88,
      percent: 12,
  ...
  it("uses the product's own VAT rate", () => {
    expect(savingsOf(100, 88, { vatRate: 13 })?.amount).toBe(13.56);
  });
});
...
describe("the space before\u00A0€", () => {
  it("is a non-breaking space, not an ordinary one", () => {
    ...
    expect(formatMoney(121.78)).toBe("121,78 €");
    expect(formatMoney(121.78)).not.toContain(" €");
  });

...
export function WidgetRenderer({
  ...
  /*
   * Widget χωρίς περιεχόμενο δεν αποδίδεται καθόλου.
   * ───────────────────────────────────────────────────────────────────────────
   * Ένα `category-card` χωρίς επιλεγμένη κατηγορία και χωρίς φόντο έβγαζε ένα
   * λευκό ορθογώνιο 509×200 πάνω από το υποσέλιδο κάθε σελίδας προϊόντος —
   * κενό, πατήσιμο, και εντελώς ανεξήγητο για τον επισκέπτη.
   *
   * Δεν είναι υποθετικό: υπήρχε ακριβώς ένα τέτοιο στη ζώνη `product.below`.
   * Ένα μισοσυμπληρωμένο widget είναι φυσιολογικό ενδιάμεσο στάδιο όσο κάποιος
   * το φτιάχνει· το κατάστημα όμως δεν πρέπει να δείχνει το ενδιάμεσο.
   *
   * Ο έλεγχος είναι «τίποτα ορατό», όχι «λείπει το τάδε πεδίο»: ένα widget με
   * μόνο εικόνα φόντου είναι απολύτως έγκυρο, όπως και ένα με μόνο τίτλο.
   */
  const hasSomethingToShow = Boolean(
    title || eyebrow || body || cta || badge || hasMedia,
  );

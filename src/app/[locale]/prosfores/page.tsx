...
export async function generateMetadata({
  ...
  const title = t("titlos_prosfores");
  const description = t("perigrafi_pragmatikes_meioseis_timis_se");
  return {
    ...
    ...pageMeta({ path: "/prosfores", locale, title, description }),
    title,
    description,
  };
}
...
export default async function OffersPage({
  ...
  /* Η σελίδα δηλώνεται ως λίστα προσφορών — αλλιώς μια μηχανή βλέπει μόνο
     «ποιο είναι το κατάστημα» και τίποτα για το τι τρέχει τώρα. */
  const collectionLd =
    offers.products.length === 0
      ? null
      : collectionJsonLd(
          {
            name: t("titlos_prosfores"),
            description: t("perigrafi_pragmatikes_meioseis_timis_se"),
            path: "/prosfores",
            items: offers.products.map((p) => ({
              name: p.name,
              path: `/proion/${p.slug}`,
            })),
          },
  ...
  return (
    <>
      ...
      <QuickViewProvider locale={locale}>
        ...
        <main id="main">
          <Zone id="offers.top" locale={locale} />
        ...
        */}
                ...
                <div className="shell-x py-9 lg:py-14">
                  ...
                  <div className="mt-8 grid gap-px border border-k-line bg-k-line lg:mt-10 lg:grid-cols-2">
                    ...
                    <div className="flex flex-col gap-3 bg-white p-5 lg:p-7">
                      ...
                      <p className="text-[12.5px] leading-[1.65] text-k-text-3">
                        {latest
                          ? t("mpikan_stin_apothiki_ton_deite", {
                              n: latest.label.toLowerCase(),
                            })
          ...
          <Zone id="offers.bottom" locale={locale} />
        </main>
